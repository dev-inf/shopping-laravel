...
class SectionFilm extends \Eloquent{

	protected $table = 'section_film';
	...
	public function fromFilm(){
      	return $this->hasOne('film', 'sec_id');
  	}

  	public function prepareData($params){
		$table = $this->table . ' AS ' . \DB::getTablePrefix() . 's';
		$tb_film = $this->tb_film . ' AS ' . \DB::getTablePrefix() . 'f';
		$available_data = \DB::table($table);
		$recordsTotal = $available_data->count();

		// seleted columns
		$select = array(\DB::raw('SQL_CALC_FOUND_ROWS "tmp"'),'s.id','s.name',\DB::raw('COUNT('.\DB::getTablePrefix().'f.id) as total_film'));
		// join here
		$available_data = $available_data->leftJoin($tb_film, 'f.sec_id', '=', 's.id');
		$available_data = $available_data->select($select);
		$available_data = $available_data->groupBy('s.id');
