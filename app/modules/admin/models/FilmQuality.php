...
class FilmQuality extends \Eloquent{

	protected $table = 'film_quality';
	...
	public function prepareData($params){
		$table = $this->table . ' AS ' . \DB::getTablePrefix() . 'q';
		$tb_film_ep = $this->tb_film_ep . ' AS ' . \DB::getTablePrefix() . 'e';
		$available_data = \DB::table($table);
		$recordsTotal = $available_data->count();

		// seleted columns
		$select = array(\DB::raw('SQL_CALC_FOUND_ROWS "tmp"'),'q.id','q.name','q.code',\DB::raw('COUNT('.\DB::getTablePrefix().'e.id) as total_film'));

		// join here
		$available_data = $available_data->leftJoin($tb_film_ep, 'e.quality_id', '=', 'q.id');
		$available_data = $available_data->select($select);
		$available_data = $available_data->groupBy('q.id');
